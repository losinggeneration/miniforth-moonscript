...
class MiniForth
	new: (subj = "", pos = 1) =>
		...
		@modes = {interpret: ->
			...
			@p_s_i!
			@interpret_primative! or @interpret_nonprimative! or @interpret_number! or error string.format[[Can't interpret: "%s"]], @word
		}
	...
	interpret_primative: => if type(@F[@word]) == "function" then
		@F[@word]!
		true

	interpret_nonprimative: => false
